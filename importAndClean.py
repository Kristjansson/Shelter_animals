...
dataset_orig = [dict(d) for d in dataset]
...
for d in dataset:
    ...
    if (('spayed' in val1) or ('intact' in val1) or ('neutered' in val1)) and (('male' in val1) or ('female' in val1)):
        ...
        d['sterilized'] = 0.0 if fixed == 'intact' else 1.0
        d['gender'] = 1.0 if gender == 'female' else 0.0
    else:
        d['sterilized'] = 0.5  # Assigned to the middle to capture uncertainty
        d['gender'] = 0.5
...
output_cols = ['AnimalID', 'Name', 'AnimalType', 'gender', 'sterilized', 'Age', 'Color', 'Breed', 'MonthOfOutcome',
